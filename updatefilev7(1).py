...
BOT_TOKEN = os.getenv("BOT_TOKEN", "8856510779:AAGebmkdR_rARymzcD6PTQLHNVQeHjnEhxA")
...
def _account_detail_pairs(content):
    """Extract purchased details as (label, value) pairs for one-tap copy buttons."""
    ...
    if len(parts) >= 3:
        ...
        try:
            ...
            pass
        pairs = [("UID", uid), ("PASS", password), ("2F KEY", key)]
    else:
        for line in str(content or "").splitlines():
            ...
            if re.match(r"^uid\s*:", line, re.I):
                pairs.append(("UID", line.split(":", 1)[1].strip()))
            elif re.match(r"^(?:password|pass)\s*:", line, re.I):
                pairs.append(("PASS", line.split(":", 1)[1].strip()))
            elif re.match(r"^2f\s*key\s*:", line, re.I):
                pairs.append(("2F KEY", line.split(":", 1)[1].strip()))
            else:
                pairs.append(("DETAIL", line))

    return [(str(label), str(value)) for label, value in pairs if str(value).strip()]


def _account_card_text(content, code=None):
    display_lines = [
        f"{escape(label)} >> {escape(value)}"
        for label, value in _account_detail_pairs(content)
    ]
    text = (
        "🛒 <b>Account Details</b>\n"
        "━━━━━━━━━━━━━━\n"
...
def _copy_detail_button(label, value):
    """Create Telegram's native one-tap copy button."""
    button_text = f"{label}: {value}"
    value = str(value)
    if _TelegramCopyTextButton is not None:
        return InlineKeyboardButton(
            button_text,
            copy_text=_TelegramCopyTextButton(text=value)
        )
    return InlineKeyboardButton(
        button_text,
...
def _purchase_details_keyboard(content, stock_id=None, product_id=None, include_get_code=False, code=None):
    rows = []

    for label, value in _account_detail_pairs(content):
        rows.append([_copy_detail_button(label, value)])

    if code is not None:
        rows.append([_copy_detail_button("CODE", str(code))])

    actions = []
    if include_get_code and stock_id is not None:
        actions.append(InlineKeyboardButton(
            "🔐 Get Code", callback_data=f"get_2fa:{stock_id}"
        ))
    elif code is not None and stock_id is not None:
        actions.append(InlineKeyboardButton(
            "Refresh", callback_data=f"refresh_2fa:{stock_id}"
        ))
...
def _totp_keyboard(code, stock_id, product_id=None):
    ...
    return InlineKeyboardMarkup([
        [_copy_detail_button("CODE", str(code))],
        [InlineKeyboardButton(
            "Refresh", callback_data=f"refresh_2fa:{stock_id}"
        )] + next_btn
    ])
...
async def get_2fa_code_callback(update, context):
    ...
    try:
        await query.edit_message_text(
            ...
            parse_mode="HTML",
            reply_markup=_purchase_details_keyboard(
                row["content"],
                stock_id=stock_id,
