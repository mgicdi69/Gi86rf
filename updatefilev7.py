...
def _purchase_details_keyboard(content, stock_id=None, product_id=None, secret=None):
    """Keyboard for purchased account details with native one-tap copy support."""
    raw_details = str(content or "").strip()
    rows = []
    if raw_details:
        if _TelegramCopyTextButton is not None:
            rows.append([InlineKeyboardButton(
                "📋 Copy All Details",
                copy_text=_TelegramCopyTextButton(text=raw_details)
            )])
        else:
            # PTB compatibility: Bot API copy_text is passed through directly.
            rows.append([InlineKeyboardButton(
                "📋 Copy All Details",
                api_kwargs={"copy_text": {"text": raw_details}}
            )])
